...
class RemainingAnalyzer:
    ...
    def analyze_stock(self, symbol, stock_num, total_stocks, start_num):
        """分析单只股票"""
        try:
            current_num = start_num + stock_num
            if stock_num % 20 == 0:  # 每20只股票报告一次进度
                progress = (stock_num / total_stocks) * 100
                print(f"📊 分析进度: {current_num}/{start_num + total_stocks} ({progress:.1f}%)")
    ...
    def analyze_remaining(self):
        ...
        print(f"\n🚀 分析剩余股票开始 - {start_time.strftime('%Y-%m-%d %H:%M:%S')}")
        print("=" * 70)
        ...
        # 跳过前100只，分析剩余的
        remaining_symbols = all_symbols[100:]  # 跳过前100只
        remaining_count = len(remaining_symbols)
        
        print(f"📊 总股票数: {total_stocks}")
        print(f"✅ 已分析: 100只")
        print(f"🎯 剩余分析: {remaining_count}只")
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            future_to_symbol = {
                executor.submit(self.analyze_stock, symbol, i+1, remaining_count, 100): symbol 
        ...
        print(f"\n📊 操作统计:")
        print(f"  • 分析成功: {successful}/{remaining_count}")
        ...
        print(f"⚡ 平均每只: {duration.total_seconds()/remaining_count:.1f}秒")
        print(f"📊 总覆盖: {100 + successful}/{total_stocks} 只股票")
        
        return results

def main():
    analyzer = RemainingAnalyzer()
    analyzer.analyze_remaining()
